...
public class TWCNativeiOSAdValidation extends TwcIosBaseTest {
	...
	public void afterTest() throws Exception {
		try {
			...
			Ad.terminateApp("com.weather.test.adPerformance");
			;
...
//		capabilities.setCapability(ReadExcelValues.data[12][0], ReadExcelValues.data[12][Cap]);
		...
		capabilities.setCapability("bundleId", "com.weather.test.adPerformance");
	...
	public void validating_Interstitial_Ad() throws Exception {
		...
		for (int i = 1; i <= 50; i++) {
			try {
				...
				Ad.findElementByAccessibilityId("Show Interstitial Ad").click();
				...
				MobileElement adElement = (MobileElement) wait
						.until(ExpectedConditions.visibilityOf(Ad.findElementByXPath(
								"//XCUIElementTypeStaticText[@name=\"You're displaying a test interstitial from\"]")));
	...
	public void validating_Marquee_Static_Ad() throws Exception {
		...
		for (int i = 1; i <= 50; i++) {
			try {
				...
				// Static Marque
				Ad.findElementByAccessibilityId("Image Marquee").click();
				long start = System.currentTimeMillis();

				MobileElement adElement = (MobileElement) wait.until(ExpectedConditions
						.visibilityOf(Ad.findElementByXPath("//XCUIElementTypeOther[@name=\"Creative Lab - MA IM\"]")));
	...
	public void validating_Marquee_Video_Ad() throws Exception {
		...
		for (int i = 1; i <= 50; i++) {
			try {
				...
				// Video Marque
				Ad.findElementByAccessibilityId("Video Marquee").click();
				long start = System.currentTimeMillis();
	...
	public void validating_Small_Banner_Ad() throws Exception {
		...
		for (int i = 1; i <= 50; i++) {
			try {
				...
				// Video Marque
				Ad.findElementByAccessibilityId("Small Banner").click();
				long start = System.currentTimeMillis();

				MobileElement adElement = (MobileElement) wait
						.until(ExpectedConditions.visibilityOf(Ad.findElementByXPath("//XCUIElementTypeLink")));
	...
	public void validating_Large_Banner_Ad() throws Exception {
		...
		for (int i = 1; i <= 50; i++) {
			try {
				...
				// Video Marque
				Ad.findElementByAccessibilityId("Large Banner").click();
				long start = System.currentTimeMillis();
